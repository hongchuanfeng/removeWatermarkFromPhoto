...
interface BarcodeRecognizeInfoProps {
  toolKey: string
  exampleImages?: string[]
}

export default function BarcodeRecognizeInfo({ toolKey, exampleImages }: BarcodeRecognizeInfoProps) {
  const { t } = useLanguage()
              ...
              <div key={index} className="bg-white rounded-lg shadow-md overflow-hidden">
                ...
                <div className="p-4">
                  <p className="text-gray-600">
                    {t(`${toolKey}.example_desc${index + 1}`)}
                  </p>
                </div>
              </div>
            ))}
